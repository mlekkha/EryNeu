...
static double *d;          /* D's used for Neville extrapolation in BuSt() */
static double *hpoints;       /* stepsizes h (=H/n) which we try in BuSt() */
static double maxdel, mindel;
static int    numdel;  /* delay parameters used by DCERk32, y_delayed */
static int gridstart;  /* for the heuristic */
static double *delay;		/* static array set in SoDe, used by DCERk32 */
static int gridpos;					/* where you are in the grid */
static double *tdone;			   /* the grid */
static double **derivv1;    	      /* intermediate derivatives 
														for the Cash
  														-Karp formula */
static double **derivv2;
static double **derivv3;
static double **derivv4;
static double **vdonne;
...
/*	printf("%d %f %f\n",fact_discons_size,fact_discons[0],fact_discons[2]);*/
	...
	for (j=0; j < defs.ngenes; j++) {
		...
		if (lp->tau[j] > maxdel) maxdel = lp->tau[j];
		if (lp->tau[j] < mindel) mindel = lp->tau[j];
...
/*		printf("The discons for [%f,%f] are %.16f %f\n",tin,tout,fact_discons[i], shift_discs[i+1]);*/
	...
	discon_array = Construct_Discont_Array(tout-tin, lp->tau,
								defs.ngenes, shift_discs, j+1, &num_discons);
...
/*		printf("%f %f m=%d\n",range/taus[i],floor(range/taus[i]),m);*/
		...
		for (k=0; k < sn; k++)
		{
			if (starts[k] > -taus[i]) {
				...
				for (j=M;j<M+m;j++) delay_array[j] = starts[k] + (j-M+1)*taus[i];
				M += m;
			}
		}
	}
...
/*	for (i=0;i<M;i++) printf("%1.14f\n",delay_array[i]);*/
	...
	qsort((void *) delay_array, M, sizeof(double),(int (*) (void*,void*)) compare);

/*	for (i=0;i<M;i++) printf("%1.14f\n",delay_array[i]);*/

	qsort((void *) taus, n, sizeof(double),(int (*)
	(void*,void*)) compare);
...
/* Now lets do the promised iteration, if required ofcourse! */
	...
	if (rktimes[3] - mindel > grid[gridsize - 1])
	{
      	...
      	d_deriv(vdone[gridsize-1], vd[0], rktimes[0], drv1, n);
...
/*			printf("Iteration No.%d, error: %f\n",it,verror_max);*/
      		...
      		for (i=0; i<n; i++) 
				vtemp[i] = vdone[gridsize-1][i] + ech 
	  					* (b21 * drv1[i]);

      		d_deriv(vtemp, vd[1], rktimes[1], drv2, n);

      		for (i=0; i<n; i++) 
				vtemp[i] = vdone[gridsize-1][i] + ech 
	  					* (b32 * drv2[i]);

      		d_deriv(vtemp, vd[2],rktimes[2], drv3, n);
      		...
      		d_deriv(vnext,vd[3], rktimes[3], drv4, n);
				...
				for (dc=0; dc < numdel; dc++) 
					if (t - tau[dc] > grid[gridsize - 1]) {
						CE(t-tau[dc], vd[vc][dc], grid[gridsize-1],
							vdone[gridsize-1], ech, 
							drv1, drv2, drv3, drv4, n);
	  		...
	  		if (it) {
      			...
      			for (i=0; i<n; i++)
					if ( fabs(vprev[i]) > 10000.*DBL_EPSILON)
	  					verror_max = 
							DMAX(fabs((vnext[i]-vprev[i])/vprev[i]), 
														verror_max);
					else
	  					verror_max = 
							DMAX(fabs((vnext[i]-vprev[i])/
								(10000.*DBL_EPSILON)), verror_max);
			}
...
was a failure, we don't have to recalculate it */
		...
		while (y_delayed(v_delayed,n,tms,delay,tdone,vdonne,
							derivv1,derivv2,derivv3,derivv4,
									gridpos+1,accuracy))
		{
...
/*				printf("dpos:%d\n",dpos);*/
  		...
  		d_deriv(vnow, v_delayed[0], t, derivv1[gridpos], n);
 ...
 * is less than the desired accuracy                                       */
	...
	/* do the Runge-Kutta thing here: calulate intermediate 
      ...
      for (i=0; i<n; i++) 
	vtemp[i] = vnow[i] + h 
	  * (b21 * derivv1[gridpos][i]);

      	d_deriv(vtemp, v_delayed[1], t + a2 * h, derivv2[gridpos], n);

      for (i=0; i<n; i++) 
	vtemp[i] = vnow[i] + h 
	  * (b32 * derivv2[gridpos][i]);

      	d_deriv(vtemp, v_delayed[2],t + a3 * h, derivv3[gridpos], n);

/* ... then feed them to the Rk32 formula */ 

      for (i=0; i<n; i++) 
	vnext[i] = vnow[i] 
	  + h * (c1 * derivv1[gridpos][i] + c2 * derivv2[gridpos][i] + c3 * derivv3[gridpos][i]);
...
succesful, it will get used as derivv1 (k1) in the next step */
      	...
      	d_deriv(vnext,v_delayed[3], t + h, derivv4[gridpos], n);

/* calculate the error estimate using the embedded formula */

      for (i=0; i<n; i++) 
	verror[i] = h * (dc1 * derivv1[gridpos][i] + dc2 * derivv2[gridpos][i] 
			    + dc3 * derivv3[gridpos][i] + dc4 * derivv4[gridpos][i]);
...
exchange the pointers */
	...
	while ((tarray[tpos] < t + h) && (tpos < tpoints)) 
	{
		...
		CE(tarray[tpos], vatt[tpos], t, vnow, h, derivv1[gridpos], derivv2[gridpos],
...
void DivideHistory(double t1, double t2)
{
	...
	if ((size = GetNNucs(t2)*defs.ngenes) > GetNNucs(t1)*defs.ngenes)
		for (i=0; i <= gridpos; i++)
		{

			blug = (double *) calloc(size, sizeof(double));

			Go_Forward(blug, vdonne[i], GetStartLinIndex(t2),
								GetStartLinIndex(t1), defs.ngenes);
			...
			Go_Forward(blug, derivv1[i], GetStartLinIndex(t2),
								GetStartLinIndex(t1), defs.ngenes);
			...
			Go_Forward(blug, derivv2[i], GetStartLinIndex(t2),
								GetStartLinIndex(t1), defs.ngenes);
			...
			Go_Forward(blug, derivv3[i], GetStartLinIndex(t2),
								GetStartLinIndex(t1), defs.ngenes);
			...
			Go_Forward(blug, derivv4[i], GetStartLinIndex(t2),
								GetStartLinIndex(t1), defs.ngenes);
